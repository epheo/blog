...
These are field notes from a TripleO-deployed RHOSP 16 environment (ML2/OVN, three
controllers, two computes). Hostnames and identifiers were anonymized but kept
consistent, so you can follow each object across the examples. The mechanics still
apply to newer OVN-based deployments, even if the services now live elsewhere.
...
.. code-block:: bash
    ...
    Container bundle set: ovn-dbs-bundle [undercloud-0.ctlplane.redhat.local:8787/rh-osbs/rhosp16-openstack-ovn-northd:pcmklatest]
     ...
     ip-172.17.1.200 (ocf::heartbeat:IPaddr2):       Started controller-0
...
.. code-block:: bash
    ...
    Resource: ovndb_servers (class=ocf provider=ovn type=ovndb-servers)
     Attributes: inactive_probe_interval=180000 manage_northd=yes master_ip=172.17.1.200 nb_master_port=6641 sb_master_port=6642
...
.. code-block:: bash
    ...
    $ ansible -b -i /usr/bin/tripleo-ansible-inventory -m shell -a "pcs status | grep ovn-dbs-bundle | grep Master" controller-0
...
.. code-block:: bash
    ...
    # On the master controller node (controller-0)
...
.. code-block:: bash
    ...
    [root@controller-1 ~]# ovs-vsctl get Open_Vswitch . external_ids:ovn-remote | cut -d':' -f 2
    172.17.1.200
...
.. code-block:: bash
    ...
    [root@controller-1 ~]# ovn-nbctl --db="tcp:172.17.1.200:6641" ls-add ovn_test_lswitch

    # Removing the test logical switch
    [root@controller-1 ~]# ovn-nbctl --db="tcp:172.17.1.200:6641" ls-del ovn_test_lswitch

    # Creating a test chassis in the SB database
    [root@controller-1 ~]# ovn-sbctl --db="tcp:172.17.1.200:6642" chassis-add ovn_test_chassis geneve 127.0.0.1

    # Removing the test chassis
    [root@controller-1 ~]# ovn-sbctl --db="tcp:172.17.1.200:6642" chassis-del ovn_test_chassis
...
Our example environment has two project networks, ``private1`` (192.168.30.0/24)
and ``private2`` (192.168.40.0/24), each with two VMs, connected by ``router1``
to the ``public`` network (10.0.0.0/24).
...
.. code-block:: bash
    ...
    ()[root@controller-0 /]# ovn-nbctl show
    [...]
...
.. code-block:: bash
    ...
    Chassis "554f96ab-2f35-40c6-a5eb-764067db3188"
        ...
        Encap geneve
            ip: "172.17.2.36"
            options: {csum="true"}
        Port_Binding "b631ec31-d634-430c-b50e-1b7819ce7dbd"  <- private1_vm1 on compute-0
        Port_Binding "a2ff1c60-c074-47c4-9c1e-c768ade269cb"  <- private2_vm1 on compute-0
    Chassis "0140f271-2811-47f5-8174-5ca281aaeb41"
        hostname: "compute-1.redhat.local"
        Encap geneve
            ip: "172.17.2.86"
            options: {csum="true"}
        Port_Binding "2d6249e2-d05c-48fd-81c6-7831cfe107b3"  <- private1_vm2 on compute-1
        Port_Binding "4b0b7e46-9217-4267-a090-418873a8b6f0"  <- private2_vm2 on compute-1
    Chassis "21347b99-e853-4aa8-b7da-82aee8aa972a"
        hostname: "controller-2.redhat.local"
        Encap geneve
            ip: "172.17.2.83"
            options: {csum="true"}
        Port_Binding "cr-lrp-5c3d686a-1918-4663-88c3-cdd9faa1d3b2"      <- Gateway port on controller-2 (SNAT traffic)
...
.. code-block:: bash
    ...
    ()[root@controller-0 /]# ovn-nbctl find ACL external_ids:"neutron\:security_group_rule_id"=0a6cfa3d-9291-4331-8e52-82b7e3800200
    ...
    label               : 0
    log                 : false
    match               : "outport == @pg_3e484201_4fba_4d47_b18c_a515b6a1b8f3 && ip4 && ip4.src == 192.168.13.116/30 && tcp && tcp.dst == 9999"
    priority            : 1002

Then, we can search for the corresponding logical flows in the Southbound database using the ACL's UUID prefix as the stage hint:

.. code-block:: bash

    ()[root@controller-0 /]# ovn-sbctl find Logical_Flow external_ids:stage-hint=fc662a6a
    _uuid               : 6e465ba7-2a0d-45c8-9401-4446d8776c71
    actions             : "next;"
    external_ids        : {source="northd.c:6108", stage-hint="fc662a6a", stage-name=ls_out_acl}
    logical_datapath    : 334f6950-34fc-474d-8897-84a2d13846a0
    match               : "reg0[8] == 1 && (outport == @pg_3e484201_4fba_4d47_b18c_a515b6a1b8f3 && ip4 && ip4.src == 192.168.13.116/30 && tcp && tcp.dst == 9999)"
    pipeline            : egress
    priority            : 2002
    table_id            : 4
    hash                : 0
    ...
    external_ids        : {source="northd.c:6084", stage-hint="fc662a6a", stage-name=ls_out_acl}
    logical_datapath    : 334f6950-34fc-474d-8897-84a2d13846a0
    match               : "reg0[7] == 1 && (outport == @pg_3e484201_4fba_4d47_b18c_a515b6a1b8f3 && ip4 && ip4.src == 192.168.13.116/30 && tcp && tcp.dst == 9999)"
    pipeline            : egress
    priority            : 2002
    table_id            : 4
    hash                : 0

Finally, we can verify the actual OpenFlow rules installed on the compute nodes that implement these logical flows. The logical flow UUID prefix becomes the OpenFlow cookie:

.. code-block:: bash

    [root@compute-0 ~]# for i in 6e465ba7 48213015; do ovs-ofctl dump-flows br-int | grep $i; done
     cookie=0x6e465ba7, duration=134139.172s, table=44, n_packets=0, n_bytes=0, idle_age=18910, hard_age=4013, priority=2002,tcp,reg0=0x100/0x100,reg15=0x3,metadata=0x8,nw_src=192.168.13.116/30,tp_dst=9999 actions=resubmit(,45)
     cookie=0x48213015, duration=128550.798s, table=44, n_packets=0, n_bytes=0, idle_age=9762, hard_age=8793, priority=2002,tcp,reg0=0x80/0x80,reg15=0x3,metadata=0x8,nw_src=192.168.13.116/30,tp_dst=9999 actions=load:0x1->NXM_NX_XXREG0[97],resubmit(,45)
...
The values `reg15=0x3,metadata=0x8` identify a particular VM port: `metadata` is the
logical datapath (the network) and `reg15` is the port's tunnel key within it; in
this case, the destination VM (192.168.13.92) on compute-0.
...
Let's use ``ovn-trace`` to explore how an ICMP packet would travel from
``private2_vm2`` (192.168.40.158, on compute-1) to ``private2_vm1``
(192.168.40.12, on compute-0):

.. code-block:: bash

    [root@compute-0 ~]# ovn-trace --db="tcp:172.17.1.200:6642" --ovs --friendly-names --ct=new private2  'inport == "4b0b7e46-9217-4267-a090-418873a8b6f0" && eth.src == fa:16:3e:fe:90:9a && eth.dst == fa:16:3e:e3:e2:c8 && ip4.src == 192.168.40.158 && ip4.dst == 192.168.40.12 && ip.ttl == 64 && icmp4.type == 8'

    #icmp,reg14=0x4,vlan_tci=0x0000,dl_src=fa:16:3e:fe:90:9a,dl_dst=fa:16:3e:e3:e2:c8,nw_src=192.168.40.158,nw_dst=192.168.40.12,nw_tos=0,nw_ecn=0,nw_ttl=64,icmp_type=8,icmp_code=0
    ...
    [...]
     9. ls_out_port_sec_l2 (ovn-northd.c:4081): outport == "private2_vm1" && eth.dst == {fa:16:3e:e3:e2:c8}, priority 50, uuid 060cf739
        cookie=0x60cf739, duration=670224.080s, table=49, n_packets=4626, n_bytes=372616, priority=50,reg15=0x3,metadata=0x3,dl_dst=fa:16:3e:e3:e2:c8 actions=resubmit(,64)
        output;
...
* Let's follow the ICMP packet from the trace above to its destination, ``private2_vm1`` on compute-0
* We ping between the two guests and expect the packet to be delivered on the correct tap interface
...
First, identify the OpenFlow port number for the VM's tap interface (the tap name
is ``tap`` followed by the first 11 characters of the Neutron port ID):

.. code-block:: bash

    [root@compute-0 ~]# ovs-ofctl show br-int | grep tapa2ff1c60
     113(tapa2ff1c60-c0): addr:fe:16:3e:e3:e2:c8
...
We can find the right DHCP_Options row in the NB database (Neutron inserts this every time a subnet is created). Here for the ``private1`` subnet:

.. code-block:: bash

    ()[root@controller-0 /]# ovn-nbctl find DHCP_Options external_ids:subnet_id=ce6193c4-80ef-448c-8153-4b8282eef0f3
...
We can also find the corresponding logical flow in the SB database. Here it matches
a DHCP request coming from ``private1_vm1``'s port:

.. code-block:: bash

    ()[root@controller-0 /]# ovn-sbctl find logical_flow external_ids:stage-name=ls_in_dhcp_options
...
Since ``private1_vm1`` runs on compute-0, we should be able to see the flow there in table 20 (12+8):

.. code-block:: bash

    [root@compute-0 ~]# ovs-ofctl dump-flows br-int | grep 6b422d1e
     cookie=0x6b422d1e, duration=780275.467s, table=20, n_packets=0, n_bytes=0, idle_age=25685, hard_age=29126, priority=100,udp,reg14=0x3,metadata=0x2,dl_src=fa:16:3e:40:31:2c,nw_src=0.0.0.0,nw_dst=255.255.255.255,tp_src=68,tp_dst=67 actions=controller(userdata=00.00.00.20.00.00.00.240.00.01.de.10.00.00.98.63.c0.a8.1e.54.79.0e.20.a9.fe.a9.fe.c0.a8.1e.02.00.c0.a8.1e.01.06.08.ac.10.00.01.0a.00.00.01.83.04.00.00.a8.c0.1a.02.05.a2.01.04.ff.ff.ff.00.03.04.c0.a8.1e.01.36.04.c0.a8.1e.01,pause),resubmit(,21)
